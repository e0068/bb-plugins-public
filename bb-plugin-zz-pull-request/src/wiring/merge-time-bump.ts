...
export async function bumpVersionsBeforeMerge(
  ...
  const planned = await planBumps(ports, repo, roots, headBranch, baseBranch);
  ...
  const { files, bumped, problems } = caughtUp
    ? await planBumps(ports, repo, roots, headBranch, baseBranch)
    : planned;
...
async function planBumps(
  ...
  for (const root of roots) {
    ...
    const plan = planMergeTimeBump(headJson, baseJson);
