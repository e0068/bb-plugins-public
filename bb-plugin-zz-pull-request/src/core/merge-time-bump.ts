...
import { bumpPatch, topLevelVersion } from "./plugin-version-bump";
 ...
 * — a plugin this branch introduces, which is ahead of nothing).
 */
export function planMergeTimeBump(
  ...
  if (compare(head, base) > 0) return { kind: "ahead" };
  return { kind: "bump", to: bumpPatch(base[0], base[1], base[2]) };
}

function parseVersion(json: string | null): Parsed | null {
  if (json === null) return null;
  const matched = PLAIN_SEMVER.exec(topLevelVersion(json) ?? "");
