...
- `server.ts` — the backend: a todo store in `bb.storage.kv`, RPC methods
  for the page, a `bb pull-request` CLI command, a setting, and a realtime signal
...
- `skills/example-todos/SKILL.md` — a skill that tells agents how to keep the list
  with `bb pull-request`. BB imports it into agent threads automatically.

Try it: install the plugin, open **Example todos** in the sidebar, then run
`bb pull-request add "Ship it"` in a terminal. The page updates at once.
...
```
bb plugin config pull-request
bb plugin config pull-request set showDone false
bb plugin reload pull-request
```
