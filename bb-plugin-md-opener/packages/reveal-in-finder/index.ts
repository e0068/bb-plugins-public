...
/**
 * Joins a repo-relative path onto an absolute root. Returns null if the
 * joined path would escape the root (e.g. a `relativePath` containing `..`
 * segments) rather than ever revealing outside it.
 */
...
/**
 * The real `execFile` an RPC handler passes to `revealInFinder`. Held as a
...
/**
 * `revealInFinder` for the current process: reads `process.platform` and
 * `revealExecFileProvider.current` itself, so an RPC handler can't forget
 * the platform gate or capture the provider once at import time (stale
 * before a test swaps it in). `revealInFinder` remains the pure-deps core
 * used directly by tests; this is what production call sites use instead.
 */
export function revealInFinderHere(absPath: string): Promise<RevealResult> {
  return revealInFinder(absPath, {
