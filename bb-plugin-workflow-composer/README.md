...
Two Claude Code workflows (`~/.claude/workflows/` engine), the same idea at two
levels of the model:

[`examples/parallel-dev.js`](examples/parallel-dev.js) — the **constructor-editable**
version. A linear `Plan → (impl(sonnet) → review(opus) → validate(haiku)) → Final`
tree that fits the composer's model, so it carries a `/* @composer-workflow */`
mirror and **opens and edits in the tree constructor**. No dynamic groups, no
disjointness, no fix loop — a starter you can grow.

[`examples/parallel-dev-full.js`](examples/parallel-dev-full.js) — the **full,
hand-written** version: a planner splits a task into file-disjoint groups
(code-level union-find), each group runs the `impl → review → validate` pipeline
...
box. Seeding is additive and never destructive — a name already present there is
left untouched, so your own edits (or a deletion, until the next name collision)
always win.
...
```
bb plugin reload workflow-composer
```

## Configure

```
bb plugin config workflow-composer
bb plugin config workflow-composer set greeting hi
```
