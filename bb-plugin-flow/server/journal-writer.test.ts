...
const setup = (overrides: { threads?: object; environments?: object; files?: object } = {}) => {
  const { bb, harness } = createFakePluginHost({
    ...
    sdk: {
      ...
      files: {
        write: async () => ({ outcome: "written" as const, sha256: "abc", sizeBytes: 10 }),
        ...overrides.files,
      },
    },
  });
...
describe("writeDecision", () => {
  ...
  it("путь настроен, дерево есть — пишет файл по имени из заголовка брифа", async () => {
    ...
    const dirs = createJournalDirStore(bb.storage.kv);
    await dirs.set("proj_1", "memory/decisions");
    expect(await writeDecision(bb, dirs, args)).toEqual({ kind: "written", path: "memory/decisions/kak-vesti-rabotu.md" });
    const [call] = harness.sdk.callsTo("files.write");
    expect(call[0]).toMatchObject({ hostId: "host_1", rootPath: "/work/tree", path: "memory/decisions/kak-vesti-rabotu.md", expectedSha256: null, createParents: true });
  });
  ...
  it("имя занято (conflict) — разводит суффиксом", async () => {
    ...
    const { bb, harness } = setup({
      files: {
        write: async () => {
          calls += 1;
          return calls === 1 ? { outcome: "conflict" as const, currentSha256: "x" } : { outcome: "written" as const, sha256: "abc", sizeBytes: 10 };
        },
      },
    });
    const dirs = createJournalDirStore(bb.storage.kv);
    await dirs.set("proj_1", "memory/decisions");
    expect(await writeDecision(bb, dirs, args)).toEqual({ kind: "written", path: "memory/decisions/kak-vesti-rabotu-2.md" });
