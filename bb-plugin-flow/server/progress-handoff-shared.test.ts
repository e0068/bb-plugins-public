...
import type { ProgressView, StageSettings } from "../shared/contract";
...
describe("ответ о передаче работы", () => {
  it("исходный и новый тред смотрят на один прогон, ведёт его новый", async () => {
    ...
    expect(((await harness.callRpc("getFlowProgress", { threadId: "thr_src" })) as ProgressView | null)?.carrier?.threadId).toBe("thr_new");
  });
});
