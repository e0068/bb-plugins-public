...
describe("место брифа не запоминается", () => {
  it("после ответа «Новый тред» следующий бриф проекта открывается на «Здесь», а маршрут нового треда помнится", async () => {
    const { harness, call } = await setup();
    await call({ briefId: "dec_1", answers: [], place: "thread", route: { tree: "local", branch: "none" } });
    expect(await harness.callRpc("getDispatchPlace", { threadId: "thr_src" })).toEqual({ place: "here", route: { tree: "local", branch: "none" } });
  });

  it("проект, где место ещё не выбирали, открывается на «Здесь» без маршрута", async () => {
