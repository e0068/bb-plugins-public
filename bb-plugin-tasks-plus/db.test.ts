...
describe("tasks storage", () => {
  ...
  it("persists workflow fields and replaces checks", async () => {
    const { store, harness } = setup();
    try {
      const project = createProject(store, "WF");
      ...
      expect(created.type).toBe("bugfix");
      expect(created.estimate).toBe("m");
      expect(created.planTokens).toBe(120);
      expect(created.factTokens).toBeNull();
      expect(store.listTaskChecks(created.id)).toEqual(["review", "test"]);

      // Update clears type, sets fact tokens, and replaces the check set.
      ...
      expect(updated.estimate).toBe("m"); // untouched
      expect(updated.factTokens).toBe(140);
      expect(store.listTaskChecks(created.id)).toEqual(["browser"]);

      // Clearing checks empties the join table.
      store.replaceTaskChecks(created.id, []);
      expect(store.listTaskChecks(created.id)).toEqual([]);
