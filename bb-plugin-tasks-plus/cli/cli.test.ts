...
describe("bb tasks CLI", () => {
  ...
  it("warns on missing process fields by default and blocks creation with --strict", async () => {
    const { bb, harness } = createFakePluginHost({ pluginId: "tasks" });
    await plugin(bb);
    ...
    // 1. Without --strict: the task is created despite missing process
    // fields, and stdout carries a warning naming exactly what's missing.
    const lenient = await harness.runCli([
      "create",
      "--project",
      "STR",
      "--title",
    ...
    expect(lenient.stdout).toContain(
      "missing process fields: type, estimate, check, plan-tokens",
    );
    expect(lenient.stdout).toContain("Created STR-1");

    // 2. With --strict: the same missing fields reject the task outright —
    // no row is created.
    const strictRejected = await harness.runCli([
      "create",
      "--project",
      "STR",
      "--title",
    ...
    expect(strictRejected.exitCode).not.toBe(0);
    expect(strictRejected.stdout).toBe("");
    expect(strictRejected.stderr).toBe(
      "missing required process fields: type, estimate, check, plan-tokens",
    );

    // 3. With --strict and every process field present, creation proceeds
    // normally (this becomes STR-2, proving STR-1's rejected sibling above
    // never consumed a key).
    const strictComplete = await harness.runCli([
      "create",
      "--project",
      "STR",
      "--title",
    ...
    expect(strictComplete.exitCode).toBe(0);
    expect(strictComplete.stderr).toBe("");
    expect(strictComplete.stdout).toContain("Created STR-2");
    expect(strictComplete.stdout).not.toContain("Warning:");

    // 4. Without --strict but with every process field present, no warning
    // is printed at all.
    const lenientComplete = await harness.runCli([
      "create",
      "--project",
      "STR",
      "--title",
      "Fully specified again",
      "--type",
      "feature",
      "--estimate",
      "s",
    ...
    expect(lenientComplete.exitCode).toBe(0);
    expect(lenientComplete.stdout).not.toContain("Warning:");

    // Only the two lenient creates and the complete strict create
    // persisted; the strict-rejected attempt left no row behind.
    const listed = JSON.parse(
      stdout(await harness.runCli(["list", "--project", "STR", "--json"])),
    );
    expect(
      listed.tasks.map((task: { key: string }) => task.key),
    ).toEqual(["STR-1", "STR-2", "STR-3"]);

    await harness.dispose();
  });

  it("sets and clears workflow fields via create and update", async () => {
    const { bb, harness } = createFakePluginHost({ pluginId: "tasks" });
    await plugin(bb);
    ...
    expect(created.task.checks).toEqual(["review", "test"]);

    const shown = stdout(await harness.runCli(["show", "wf-1"]));
    expect(shown).toContain("Type");
    expect(shown).toContain("bugfix");
    expect(shown).toContain("Plan tokens");
    ...
    const afterUpdate = JSON.parse(
      stdout(await harness.runCli(["show", "wf-1", "--json"])),
    );
    expect(afterUpdate.task).toMatchObject({
      type: null,
      estimate: "m",
      factTokens: 140,
    });
    expect(afterUpdate.task.checks).toEqual(["browser"]);
    ...
    expect(cleared.task.checks).toEqual([]);
    expect(cleared.task.planTokens).toBeNull();

    await expect(
      harness.runCli(["update", "wf-1", "--plan-tokens", "-5"]),
