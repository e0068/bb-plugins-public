...
const PROJECT_HELP = `Usage:
  ...
  bb tasks project update <prefix-or-id> [--name <name>] [--color <color>] [--folder <id-or-name> | --no-folder] [--link-bb-project <proj_id> | --unlink-bb-project] [--tasks-folder <repo-relative-path> | --no-tasks-folder] [--rename-prefix X] [--json]`;
...
async function runProject(
  ...
  if (action === "update") {
    assertAllowed(
      ...
      ["no-folder", "unlink-bb-project", "no-tasks-folder"],
    );
    const [address] = requirePositionals(
      args,
      1,
    ...
    const tasksFolderOpt = option(args, "tasks-folder");
    ...
    validateSingleFlagChoice(
      tasksFolderOpt,
      args.flags.has("no-tasks-folder"),
      "tasks-folder",
      "no-tasks-folder",
    );
    const folder = folderAddress
    ...
    const changes = {
      ...
      linkedBbProjectId: args.flags.has("unlink-bb-project")
        ? null
        : linkedBbProjectId,
      tasksFolder: args.flags.has("no-tasks-folder") ? null : tasksFolderOpt,
    };
    ...
    const updated = store.transaction(() =>
      store.tasks.updateProject(project.id, {
        ...
        tasksFolder: updateInput?.tasksFolder,
      }),
    );
...
async function runCreate(
  ...
  if (args.flags.has("strict") && missingProcessFields.length > 0) {
    return {
      exitCode: 1,
      stdout: "",
      stderr: `missing required process fields: ${missingProcessFields.join(", ")}`,
    };
  }
  ...
  const processFieldsWarning =
    missingProcessFields.length > 0
      ? `missing process fields: ${missingProcessFields.join(", ")}`
      : undefined;
  const stdout = args.flags.has("json")
    ? json({
        ...
        attachments,
        failedAttachments,
        warnings: processFieldsWarning ? [processFieldsWarning] : [],
      })
    : [
        `Created ${task.key}  ${task.title}`,
        ...(processFieldsWarning ? [`Warning: ${processFieldsWarning}`] : []),
        ...attachments.map(
...
async function runUpdate(
  ...
  const estimateOpt = option(args, "estimate");
  validateSingleFlagChoice(
    estimateOpt,
