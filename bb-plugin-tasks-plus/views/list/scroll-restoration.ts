...
export function listScrollScopeKey(params: {
  ...
  const types = JSON.stringify([...params.filters.types].sort());
  const estimates = JSON.stringify([...params.filters.estimates].sort());
  const labels = JSON.stringify([...params.filters.labelNames].sort());
  return `${list}|s=${statuses}|p=${priorities}|ty=${types}|e=${estimates}|l=${labels}|sort=${params.sort}`;
}
