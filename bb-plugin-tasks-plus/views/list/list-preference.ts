 ...
 */
function uniqueValidTypes(values: unknown): TaskType[] {
  if (!Array.isArray(values)) return [];
  const seen = new Set<TaskType>();
  const result: TaskType[] = [];
  for (const value of values) {
    if (typeof value !== "string" || !TYPE_SET.has(value)) continue;
...
function uniqueValidEstimates(values: unknown): TaskEstimate[] {
  if (!Array.isArray(values)) return [];
  const seen = new Set<TaskEstimate>();
  const result: TaskEstimate[] = [];
  for (const value of values) {
    if (typeof value !== "string" || !ESTIMATE_SET.has(value)) continue;
    const estimate = value as TaskEstimate;
    if (seen.has(estimate)) continue;
    seen.add(estimate);
...
function sanitizeSort(value: unknown): ListSort {
...
export function sanitizeListPreference(raw: unknown): ListPreference {
  ...
  return {
    filters: {
      ...
      types: uniqueValidTypes(filtersRaw.types),
      estimates: uniqueValidEstimates(filtersRaw.estimates),
