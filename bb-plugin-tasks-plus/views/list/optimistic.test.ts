...
describe("matchesFilters", () => {
  ...
  it("filters by type and estimate, excluding tasks without a value", () => {
    const typed = task({ id: "T2", type: "bugfix", estimate: "m" });
    expect(matchesFilters(typed, [], [], [], ["bugfix"], [])).toBe(true);
    expect(matchesFilters(typed, [], [], [], ["feature"], [])).toBe(false);
    expect(matchesFilters(typed, [], [], [], [], ["m", "l"])).toBe(true);
    expect(matchesFilters(typed, [], [], [], [], ["xl"])).toBe(false);
    // A task with no type/estimate is excluded by an active filter.
    expect(matchesFilters(t, [], [], [], ["bugfix"], [])).toBe(false);
    expect(matchesFilters(t, [], [], [], [], ["m"])).toBe(false);
  });
});
