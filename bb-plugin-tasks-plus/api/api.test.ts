...
// revealTaskSource shells out to `open -R` (node:child_process.execFile).
// The RPC-level tests below swap in a fake execFile via the provider seam
// (see reveal/index.ts) so the real handler wiring is exercised without ever
// spawning a real Finder process from a test run.
const revealExecFile = {
  calls: [] as { file: string; args: readonly string[] }[],
};

describe("Tasks RPC domain API", () => {
  beforeEach(() => {
    revealExecFile.calls.length = 0;
    revealExecFileProvider.current = (file, args, callback) => {
      revealExecFile.calls.push({ file, args });
      callback(null);
    };
  });
  ...
  it("reveals a real file_tasks link's source in Finder via open -R", async () => {
    const { bb, harness } = createFakePluginHost({
      pluginId: "tasks",
      sdk: {
        projects: {
          get: async () => ({
            sources: [
              { hostId: "host_1", path: "/repo", isDefault: true },
            ],
          }),
        },
        system: {
          config: async () => ({ primaryHostId: "host_1" }),
        },
      },
    });
    const store = createStore(bb);
    ...
    const fetched = tasksRpcContract.getTask.output.parse(
      await harness.callRpc("getTask", { taskId: task.id }),
    );
    ...
    const reveal = tasksRpcContract.revealTaskSource.output.parse(
      await harness.callRpc("revealTaskSource", { taskId: task.id }),
    );
    expect(reveal).toEqual({ revealed: true, error: null });
    expect(revealExecFile.calls).toEqual([
      {
        file: "open",
        args: ["-R", "/repo/memory/tasks/backlog/wgsl-e2-groups.md"],
      },
    ]);
  ...
  it("refuses to reveal when the bb project's source lives on a non-primary host", async () => {
    const { bb, harness } = createFakePluginHost({
      pluginId: "tasks",
      sdk: {
        projects: {
          get: async () => ({
            sources: [
              { hostId: "host_remote", path: "/repo", isDefault: true },
            ],
          }),
        },
        system: {
    ...
    expect(reveal.error).not.toBeNull();
    expect(revealExecFile.calls).toEqual([]);
  ...
  it("does not fall back to a legacy 'Source: … · slug: …' description marker when file_tasks has no link", async () => {
    ...
    // text. (The marker parser itself lives on only for that sync-time
    // adoption pass — see filesync/sync.test.ts.)
    const { bb, harness } = createFakePluginHost({ pluginId: "tasks" });
    const store = createStore(bb);
    registerTasksApi(bb, store);
    const project = store.tasks.createProject({
      ...
      prefix: "SL",
      color: "teal",
      linkedBbProjectId: "proj_shader_lab",
    });
    const task = store.tasks.createTask({
      projectId: project.id,
      title: "WGSL groups",
    ...
    expect(fetched.task?.source).toBeNull();
  ...
  it("uses the real file_tasks link even when a stale legacy description marker is also present", async () => {
    const { bb, harness } = createFakePluginHost({ pluginId: "tasks" });
    const store = createStore(bb);
    registerTasksApi(bb, store);
    const project = store.tasks.createProject({
  ...
  it("has no clickable source when neither file_tasks nor a description marker exist", async () => {
    const { bb, harness } = createFakePluginHost({ pluginId: "tasks" });
    const store = createStore(bb);
    registerTasksApi(bb, store);
    const project = store.tasks.createProject({
