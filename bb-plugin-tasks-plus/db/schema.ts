  ...
  `
    ALTER TABLE tasks ADD COLUMN type TEXT
      CHECK (type IS NULL OR type IN (
        'feature', 'bugfix', 'spike', 'refactor', 'migration', 'design'
      ));
    ALTER TABLE tasks ADD COLUMN estimate TEXT
      CHECK (estimate IS NULL OR estimate IN ('xs', 's', 'm', 'l', 'xl'));
    ALTER TABLE tasks ADD COLUMN plan_tokens INTEGER
      CHECK (plan_tokens IS NULL OR plan_tokens >= 0);
    ALTER TABLE tasks ADD COLUMN fact_tokens INTEGER
      CHECK (fact_tokens IS NULL OR fact_tokens >= 0);

    CREATE TABLE task_checks (
      task_id TEXT NOT NULL REFERENCES tasks(id) ON DELETE CASCADE,
      check_name TEXT NOT NULL CHECK (
        check_name IN ('test', 'review', 'design', 'browser')
      ),
      PRIMARY KEY (task_id, check_name)
    );
    CREATE INDEX idx_task_checks_task ON task_checks(task_id);

    CREATE TRIGGER task_list_revision_checks_insert
    AFTER INSERT ON task_checks BEGIN
      UPDATE task_list_revision SET revision = revision + 1 WHERE id = 1;
    END;
    CREATE TRIGGER task_list_revision_checks_delete
    AFTER DELETE ON task_checks BEGIN
      UPDATE task_list_revision SET revision = revision + 1 WHERE id = 1;
    END;
  `,
  `
    ALTER TABLE projects ADD COLUMN tasks_folder TEXT;
