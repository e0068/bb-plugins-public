...
function normalizeTasksFolder(folder: string | null): string | null {
  if (folder === null) return null;
  const trimmed = folder.trim().replace(/^\/+|\/+$/g, "");
  if (trimmed.length === 0) return null;
  if (trimmed.startsWith("/") || trimmed.split("/").includes("..")) {
    throw new Error("tasksFolder must be a relative path without '..'");
  }
  return trimmed;
}
...
export function createTasksStore(db: PluginDatabase) {
  ...
  function createProject(input: CreateProjectInput): Project {
    ...
    >(
      ...
      INSERT INTO projects
        (id, name, prefix, next_task_number, color, folder_id, linked_bb_project_id, tasks_folder, created_at)
      VALUES (?, ?, ?, 1, ?, ?, ?, ?, ?)
    `,
    ).run(
      id,
      ...
      normalizeTasksFolder(input.tasksFolder ?? null),
      nowIso(),
    );
  ...
  function updateProject(id: string, input: UpdateProjectInput): Project {
    ...
    `,
      ...
      input.tasksFolder === undefined
        ? current.tasksFolder
        : normalizeTasksFolder(input.tasksFolder),
      id,
    );
    return requireProject(id);
  }
  ...
  const createTaskTransaction = db.transaction(
    (input: CreateTaskInput): Task => {
      ...
      INSERT INTO tasks (
        ...
        type, estimate, plan_tokens, fact_tokens, due_date,
    ...
    `,
        ...
        input.type ?? null,
        input.estimate ?? null,
        validateTokenCount(input.planTokens ?? null, "planTokens"),
        validateTokenCount(input.factTokens ?? null, "factTokens"),
      ...
      if (input.checks !== undefined) {
        replaceTaskChecks(id, input.checks);
      }
  ...
  const updateTaskTransaction = db.transaction(
    (id: string, input: UpdateTaskInput): Task => {
      ...
      `,
        ...
        input.estimate === undefined ? current.estimate : input.estimate,
        input.planTokens === undefined
          ? current.planTokens
          : validateTokenCount(input.planTokens, "planTokens"),
        input.factTokens === undefined
          ? current.factTokens
          : validateTokenCount(input.factTokens, "factTokens"),
  ...
  function listTaskChecks(taskId: string): TaskCheck[] {
    return db
      .prepare<[string], { check_name: TaskCheck }>(
        "SELECT check_name FROM task_checks WHERE task_id = ? ORDER BY check_name",
      )
      .all(taskId)
      .map((row) => row.check_name);
  }

  const replaceTaskChecksTransaction = db.transaction(
    (taskId: string, checks: readonly TaskCheck[]): void => {
      const unique = Array.from(new Set(checks));
      for (const check of unique) {
        if (!TASK_CHECKS.includes(check)) {
          throw new Error(`Unknown check: ${String(check)}`);
        }
      }
      db.prepare<[string]>("DELETE FROM task_checks WHERE task_id = ?").run(
        taskId,
      );
      const insert = db.prepare<[string, TaskCheck]>(
        "INSERT INTO task_checks (task_id, check_name) VALUES (?, ?)",
      );
      for (const check of unique) insert.run(taskId, check);
    },
  );

  function replaceTaskChecks(
    taskId: string,
    checks: readonly TaskCheck[],
  ): void {
    replaceTaskChecksTransaction(taskId, checks);
  }

  function listSyncProjects(): Project[] {
    return db
      .prepare<[], ProjectRow>(
        `SELECT * FROM projects
         WHERE tasks_folder IS NOT NULL AND linked_bb_project_id IS NOT NULL
  ...
  function getFileTask(projectId: string, slug: string): FileTask | undefined {
    const row = db
      .prepare<
        [string, string],
        FileTaskRow
      >("SELECT * FROM file_tasks WHERE project_id = ? AND slug = ?")
      .get(projectId, slug);
    return row ? fileTaskFromRow(row) : undefined;
  }

  function getFileTaskByTaskId(taskId: string): FileTask | undefined {
    const row = db
      .prepare<
        [string],
        FileTaskRow
      >("SELECT * FROM file_tasks WHERE task_id = ?")
      .get(taskId);
    return row ? fileTaskFromRow(row) : undefined;
  }

  function listFileTasks(projectId: string): FileTask[] {
    return db
      .prepare<
        [string],
        FileTaskRow
      >("SELECT * FROM file_tasks WHERE project_id = ? ORDER BY slug")
      .all(projectId)
      .map(fileTaskFromRow);
  }

  function upsertFileTask(input: {
    projectId: string;
    slug: string;
    taskId: string;
    filePath: string;
  ...
  function deleteFileTask(projectId: string, slug: string): void {
    db.prepare<[string, string]>(
      "DELETE FROM file_tasks WHERE project_id = ? AND slug = ?",
    ).run(projectId, slug);
  }

  /**
   * Unlinks every file-synced task in a project without deleting the tasks
   * themselves — the "disconnect a folder, keep its tasks" path. Returns the
   * number of links removed.
   */
  function clearFileTasksForProject(projectId: string): number {
    return db
      .prepare<
        [string]
      >("DELETE FROM file_tasks WHERE project_id = ?")
