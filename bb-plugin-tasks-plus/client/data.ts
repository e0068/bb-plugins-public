...
/**
 * Typed RPC client bound to the tasks contract. Внутри поверхности треда
 * (`CallerThreadProvider`) задачные вызовы уезжают с его `callerThreadId`, и
 * сервер читает рабочее дерево этого треда; на доске провайдера нет, поэтому
 * вызовы идут как раньше и работают с main. Виды об этом не знают — иначе о
 * треде пришлось бы помнить в каждом из пятнадцати файлов, зовущих клиент.
 */
  ...
  return useMemo(
    () => ({
      call: ((method: string, ...args: unknown[]) =>
        (rpc.call as (method: string, ...args: unknown[]) => Promise<unknown>)(
          method,
          withCallerThreadInput(method, args[0], callerThreadId),
        )) as typeof rpc.call,
    }),
    [rpc, callerThreadId],
  );
